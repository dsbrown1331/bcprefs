...
from spinup.algos.pytorch.ppo.bc import train_good_bad_bc_policy, evaluate_policy
...
if __name__ == '__main__':
    ...
    for num_bad_demos in range(0,len(bad_demos)+1):
        ...
        pi = train_good_bad_bc_policy(good_obs, good_acs, bad_obs, bad_acs, env, args.BC_iters, args.hid_size, args.num_layers, args.pi_lr)        # pi = train_bc_policy(demo_obs, demo_acs, env, args.BC_iters, args.hid_size, args.num_layers, args.pi_lr)
    ...
    pickle.dump(results_bad_degredation, open( os.path.join(args.save_path, "goodbad_{}.p".format(save_file_name)), "wb" ))
